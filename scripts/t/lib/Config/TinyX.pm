	...
	for my $section ( sort { (($b eq '_') <=> ($a eq '_')) || ($a cmp $b) } keys %$self )
	{
		...
		for my $property ( @$block )
		{
			if ($property->{comment}) {
				...
				foreach my $c (@{$property->{comment}}) {
				    $contents .= "$c\n" if $c;
				    delete $property->{comment};
				}
			}

			foreach my $k (keys %$property) {

			    # return $self->_error("Illegal newlines in property '$section.$property'") if $block->{$property} =~ /(?:\012|\015)/s;

			    # $contents .= "$property=$block->{$property}\n";

			    return $self->_error("Illegal newlines in property '$section.$property->{$k}'") if $property->{$k} =~ /(?:\012|\015)/s;

			    $set->{$section}->{$k} = $property->{$k};

			    $contents .= "$k=$property->{$k}\n";
			}
		}
	}

	$Config::TinyX::set = $set;
...
sub set { $Config::TinyX::set }
sub errstr { $Config::TinyX::errstr }
sub _error { $Config::TinyX::errstr = $_[1]; undef }
...
Config::TinyX - Read/Write .ini style files with as little code as possible. Please note this module is inherit from Config::TinyX. It is improved to be capable to append config comments in order.
...
=head1 SYNOPSIS
	...
	# Create a config
	my $Config = Config::TinyX->new;

	# Open the config
	$Config = Config::TinyX->read( 'file.conf' );
	$Config = Config::TinyX->read( 'file.conf', 'utf8' ); # Neither ':' nor '<:' prefix!
	$Config = Config::TinyX->read( 'file.conf', 'encoding(iso-8859-1)');
	...
	my($config) = Config::TinyX -> read_string('alpha=bet');
	my($value)  = $$config{_}{alpha}; # $value is 'bet'.

	my($config) = Config::TinyX -> read_string("[init]\nalpha=bet");
	my($value)  = $$config{init}{alpha}; # $value is 'bet'.

=head1 DESCRIPTION

C<Config::TinyX> is a Perl class to read and write .ini style configuration
...
The C<*::TinyX> family of modules is specifically intended to provide an ultralight alternative
...
Lastly, L<Config::TinyX> does B<not> preserve your comments, whitespace, or the order of your config
file.

See L<Config::TinyX::Ordered> (and possibly others) for the preservation of the order of the entries
in the file.
...
You can also retrieve the error message from the C<$Config::TinyX::errstr> variable.

=head2 new()

The constructor C<new> creates and returns an empty C<Config::TinyX> object.
...
C<Config::TinyX> object containing the properties in the file.
...
When C<read> fails, C<Config::TinyX> sets an error message internally
you can recover via C<Config::TinyX-E<gt>errstr>. Although in B<some>
...
as a string and returns the C<Config::TinyX> object for it.
...
Try:
	...
	perl -MData::Dumper -MConfig::TinyX -E 'my $c=Config::TinyX->read_string("one=s"); say Dumper $c'
...
Or even, a bit ridiculously:
	...
	my($value) = ${Config::TinyX -> read_string('alpha=bet')}{_}{alpha}; # $value is 'bet'.
...
C<Config::TinyX> will only recognize the first time an option is set in a
...
L<http://rt.cpan.org/NoAuth/ReportBug.html?Queue=Config-TinyX>
